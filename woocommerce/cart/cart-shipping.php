...
<div class="shipping">
	<div><?php
		if ( $show_package_details ) {
			printf( __( 'Pengiriman #%d', 'woocommerce' ), $index + 1 );
		} else {
			_e( 'Pengiriman', 'woocommerce' );
		}
	?></div>
	<div>
				...
				<div id="shipping_method">
					<?php foreach ( $available_methods as $method ) : ?>
						<div class="p-short-detail">
							<input type="radio" name="shipping_method[<?php echo $index; ?>]" data-index="<?php echo $index; ?>" id="shipping_method_<?php echo $index; ?>_<?php echo sanitize_title( $method->id ); ?>" value="<?php echo esc_attr( $method->id ); ?>" <?php checked( $method->id, $chosen_method ); ?> class="radio-btn-ship p-short-detail" />
		...
		<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php else : ?>
				...
				<p><?php _e( 'Waiting shipping ...', 'woocommerce' ); ?></p>
